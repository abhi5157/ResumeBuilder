...
def render_live_preview_panel(profile_data: Dict):
    ...
    st.markdown('<p class="preview-section-title">Volunteer Experience</p>', unsafe_allow_html=True)
    volunteer_experience = st.session_state.get("volunteer_experience", [])
    ...
    if volunteer_experience:
        for vol in volunteer_experience:
            ...
            # Build subtitle with location and duration
            subtitle_parts = []
            if location:
            ...
            st.markdown(f"""
                <div class="preview-card">
                    <p class="preview-card-title">{role} - {organization}</p>
                    {f'<p class="preview-card-subtitle">{subtitle}</p>' if subtitle else ''}
                </div>
            """, unsafe_allow_html=True)
    else:
        st.markdown('<div class="preview-empty">Volunteer experience will appear here</div>', unsafe_allow_html=True)
